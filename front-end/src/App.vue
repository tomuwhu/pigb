<template>
  <div id="app">
    ...
    <select v-model="o.iskola">
      ...
      <option
        value="5"
      >Szegedi SzC Krúdy Gyula Kereskedelmi, Vedéglátóipari és Turisztikai Szakgimnáziuma és Szakközépiskolája (062105)</option>
    ...
    <table class="onlyprint">
      <tr>
        <th>#</th>
        <th>Inzézmény neve &nbsp;</th>
        <th>Cím</th>
        <th>Kapcsolattartó</th>
      </tr>
      <tr :key="index" v-for="(partner,index) in o.pt" v-show="partner.pintnev">
        <td>{{index+1}}.</td>
        <td>{{partner.pintnev}} &nbsp;</td>
        <td>
          {{partner.cim}}
          {{partner.porszag}} &nbsp;
        </td>
        <td>{{partner.ktneve}} &nbsp;</td>
      </tr>
    </table>
    <div style="text-align:center;" class="no-print">
      <button
        v-if="!o.pt.filter(v => !v.pintnev ).length"
        @click="o.pt.push({})"
      >+ további partner hozzáadása +</button> &nbsp;
      <button
        v-if="o.pt.filter(v => !v.pintnev ).length"
        @click="ust()"
      >- Üres sorok törlése -</button>
    </div>
    ...
    <span v-if="o.pp!=='epp'">
      ...
      <input class="ir" type="number" min="0" v-model="o.oszk" pattern> &nbsp;
      <span v-if="o.penznem">{{o.penznem}}</span>
      <span v-else>HUF</span>
      <h3>3.3 Önerő mértéke (amennyiben releváns):</h3>
      <input class="ir" type="number" min="0" v-model="o.on"> &nbsp;
      <span v-if="o.penznem">{{o.penznem}}</span>
      <span v-else>HUF</span>
      ...
      <span v-if="reszfin>0">
        ...
        <input class="ir" :value="reszfin" disabled> HUF
        <br>
      </span>
    </span>
    ...
    <table border="0">
      <td style="width: 600px;">
        Kelt,
        <input type="date" v-model="o.kelt">
      </td>
      <td style="text-align:center;" class="onlyprint">____________________
        <br>igazgató
      </td>
    </table>
    ...
    <table style="width:100%;">
      ...
      <tr>
        <td class="negyed"></td>
        <td class="negyed">
          <h4 class="center">igen</h4>
          <br>
          <br>
        </td>
        <td class="negyed">
          <h4 class="center">nem</h4>
          <br>
          <br>
        </td>
        <td class="negyed"></td>
      ...
      <tr>
        <td colspan="2">Kelt: Szeged,................</td>
        <td class="center" colspan="2">..................................
          <br>Angyalné Kovács Anikó
          <br>főigazgató
          <br>
        </td>
      </tr>
...
export default {
  ...
  computed: {
    reszfin() {
      return (Number(this.o.ito) * (1 - Number(this.o.szazalek) / 100)).toFixed(
        0
      );
    }
  },
  methods: {
    getlist() {
      this.axios.get(server_adress + "listall/").then(resp => {
    ...
    loadfromserver(id) {
      this.axios.get(server_adress + "get/" + id).then(resp => {
